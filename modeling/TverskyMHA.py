...
class TverskyVarlenMHA(nn.Module):
    def __init__(self, config, layer_idx=0):
        ...
        self.kv_heads = config.n_kv_heads
        ...
        self.shared_features = nn.Parameter(torch.empty(config.attn_head_proj_size, config.attn_shared_proj_size))
    ...
    def forward(self, x, cu_seqlens=None, max_seqlen=None):
        ...
        # This is actually GQA
        # Where s -> total sequence length (of all batches combined), h -> number attn heads, d -> hidden dimension
        q = rearrange(q, 's (h d) -> s h d', h=self.n_heads)      # s x 16 x 32
        k = rearrange(k, 's (h d) -> s h d', h=self.kv_heads)     # s x 4 x 32
        v = rearrange(v, 's (h d) -> s h d', h=self.kv_heads)     # s x 4 x 32
